...
public:
    ...
    */
    void Delete( const TK& key )
    {
        ...
        {
            // Right leg of node we are deleting
            Node<TK,TD>* tempNode = node->ptrRight;
            // Smallest node in right leg.
            ...
            // If the smallest wasn't the first one on the right
            // point the parent to whatever the smallest had on it's
            // right side. (There won't be anything on the left.)
            // Also, point the new node to the right subtree.
            ...
            // If it was the root node, replace it here.
            if (node == m_ptrRoot)
            {
                m_ptrRoot = successor;
            }
            // Otherwise we have to fix the parent of the node we replaced.
            else if (isLeftNode)
            {
                node = parent->ptrLeft;
        ...
        // Clean up the pointer we are deleting.
...
private:
    ...
    */
    Node<TK, TD>* FindParentOfNode( const TK& key )
    {
		...
		while (parent != nullptr)
		{
			...
			if ((parent->ptrLeft != nullptr && parent->ptrLeft->key == key ) ||
				(parent->ptrRight != nullptr && parent->ptrRight->key == key ))
			{
				return parent;
			}
			else
			{
				key < parent->key ?
					parent = parent->ptrLeft :
					parent = parent->ptrRight;
			}
		}
		return nullptr;
    }
    ...
    */
    TK* GetMax( Node<TK, TD>* ptrCurrent )
    {
        ...
        return ptrCurrent->ptrRight ? GetMax(ptrCurrent->ptrRight): &ptrCurrent->key; 
    }
    ...
    */
    int GetHeight( Node<TK, TD>* ptrCurrent )
    {
		...
		{
			int left = GetHeight(ptrCurrent->ptrLeft);
			int right = GetHeight(ptrCurrent->ptrRight);
