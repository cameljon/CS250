...
{
    int max = 0;
    cout << "What is the max size of the airplane? : ";
    cin >> max;
    m_airport.SetMaxCapacity( max );
    ...
    while ( m_state != FINISHED )
    {
        ...
        else if ( m_state == DISEMBARKING )
        {
            bool disembark = Disembark();
            if ( disembark )
            {
                m_state = FINISHED;
            }
...
{
    int person = rand() % 1000;
    Traveller* traveller = m_pplManager.GetTraveller( person );
